...
FactPanel {
    ...
    Column {
        ...
        VehicleSummaryRow {
            labelText: qsTr("指南针:")
            valueText: mag0IdFact ? (mag0IdFact.value  === 0 ? qsTr("需要设定") : qsTr("就绪")) : ""
        }

        VehicleSummaryRow {
            labelText: qsTr("陀螺仪:")
            valueText: gyro0IdFact ? (gyro0IdFact.value === 0 ? qsTr("需要设定") : qsTr("就绪")) : ""
        }

        VehicleSummaryRow {
            labelText: qsTr("加速度计:")
            valueText: accel0IdFact ? (accel0IdFact.value === 0 ? qsTr("需要设定") : qsTr("就绪")) : ""
        }

        VehicleSummaryRow {
            labelText: qsTr("空速计:")
            valueText: dpressOffFact ? (dpressOffFact.value === 0 ? qsTr("需要设定") : qsTr("就绪")) : ""
        }
    }
}
