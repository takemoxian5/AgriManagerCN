...
Item {
    ...
    readonly property string title:                     qsTr("飞行模式")

    property string topHelpText: ScreenTools.isTinyScreen ?
                                     qsTr("为遥控器控制通道分配飞行模式并调整触发信号的阈值.") :
                                     (qsTr("为遥控器控制通道分配飞行模式并调整触发信号的阈值. ") +
                                      qsTr("你可以将多个飞行模式分配到单个通道. ") +
                                      qsTr("打开遥控器测试通道模式设置. ") +
                                      qsTr("以下通道: ") + controller.reservedChannels +
                                      qsTr(" 无法用于飞行模式，因为它们已经用于其他功能."))
    ...
    Component {
        ...
        QGCViewMessage {
            message: qsTr("由于启用了摇杆，所以禁用了飞行模式配置.")
        }
    }
