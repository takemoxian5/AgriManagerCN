...
QGCView {
    ...
    MessageDialog {
        ...
        text:   qsTr("摇杆操控需要MAVLink手动控制支持. ") +
                qsTr("你正在运行的固件通常不支持这个. ") +
                qsTr("只有修改了固件添加手动控制支持，它才会起作用.")
    }
        ...
        if (vehicleArmed) {
            ...
            if (promptForMissionRemove && (_missionController.containsItems || _geoFenceController.containsItems || _rallyPointController.containsItems)) {
                ...
                if (!_activeVehicle.apmFirmware) {
                    root.showDialog(missionCompleteDialogComponent, qsTr("飞行计划完成"), showDialogDefaultWidth, StandardButton.Close)
                }
            }
            ...
            QGCFlickable {
                ...
                ColumnLayout {
                    ...
                    QGCLabel {
                        ...
                        text:                   qsTr("%1 图像拍摄").arg(_activeVehicle.cameraTriggerPoints.count)
