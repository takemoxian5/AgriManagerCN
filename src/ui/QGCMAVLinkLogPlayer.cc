...
void QGCMAVLinkLogPlayer::_selectLogFileForPlayback(void)
{
    ...
    if (qgcApp()->toolbox()->multiVehicleManager()->activeVehicle()) {
        QGCMessageBox::information(tr("日志重放"), tr("在重新播放日志之前，必须关闭所有连接."));
        return;
    }
    ...
    LogReplayLinkConfiguration* linkConfig = new LogReplayLinkConfiguration(QString("日志重放"));
...
{
    _ui->logFileNameLabel->setText("错误");
    _enablePlaybackControls(false);
}
