...
Item {
    ...
    Component {
        ...
        QGCMapLabel {
            ...
            map:                    _root.map
            text:                   qsTr("点击添加点 %1").arg(ScreenTools.isMobile || !polygonReady ? "" : qsTr("- 右击结束多边形"))
            ...
            Connections {
                ...
                onPolygonReadyChanged: {
                    if (polygonReady && !ScreenTools.isMobile) {
                        polygonHelp.text = qsTr("点击添加点-右击结束多边形")
                    }
                }

                onAdjustingPolygonChanged: {
                    if (adjustingPolygon) {
                        polygonHelp.text = qsTr("通过拖拽来调整多边形")
                    }
